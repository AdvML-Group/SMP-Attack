...
- `./cpp/SLICSP.cpp`: C++ core implementation of Semantics-aware Multi-granularity PatchOut, used to accelerate attack computations. Compiled into a shared library (e.g., SLICSP.so) with "g++ SLICSP.cpp -fPIC -shared -o SLICSP.so", and call the resulting library from attack_SMP.py.
